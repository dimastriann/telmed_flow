...
class PatientBooking(http.Controller):
    ...
    def patient_booking(self):
        ...
        values = {"user": request.env.user.name, "date": fields.Date.today()}
        return request.render("base_hospital_management.patient_booking_form", values)

    @http.route("/patient_booking/success", type="http", website=True, csrf=False)
    def patient_booking_submit(self, **kw):
        """Function for submitting the patient booking"""
        if request.env.user.partner_id.patient_seq in ["New", "User", "Employee"]:
            request.env.user.partner_id.sudo().write(
                {
                    "patient_seq": request.env["ir.sequence"]
                    .sudo()
                    .next_by_code("patient.sequence")
                }
            ) or "New"
        outpatient = (
    ...
    @http.route(
        "/patient_booking/get_doctors", type="json", auth="public", website=True
    )
    def update_doctors(self, **kw):
        ...
        if kw.get("department"):
            domain.append(("doctor_id.department_id.id", "=", kw.get("department")))
        ...
        for rec in allocation:
            if request.env.user.partner_id not in rec.mapped("op_ids.patient_id"):
                ...
                if {
                    ...
                    departments.append(
                        {"id": rec.department_id.id, "name": rec.department_id.name}
                    )
