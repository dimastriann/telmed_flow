...
class ViewPortal(http.Controller):
    ...
    def portal_my_vaccine(self, **kw):
        ...
        vaccination_list = []
        for rec in (
            request.env["hospital.vaccination"]
            .sudo()
            .search([("patient_id.user_ids.id", "=", request.uid)])
        ):
        ...
        values = {"vaccinations": vaccination_list, "page_name": "vaccination"}
        return request.render("base_hospital_management.portal_my_vaccines", values)
    ...
    def portal_my_tests(self, **kw):
        ...
        tests_list = []
        for rec in (
            request.env["patient.lab.test"]
            .sudo()
            .search([("patient_id.user_ids", "=", request.uid)])
        ):
            request.env["account.move"].sudo().search(
                [("ref", "=", rec.test_id.name)], limit=1
            )
            data = {"id": rec.id, "name": rec.test_id.name, "date": rec.date}
            tests_list.append(data)
        values = {"tests": tests_list, "page_name": "lab_test"}
        return request.render("base_hospital_management.portal_my_tests", values)

    @http.route("/my/tests/<int:test_id>", type="http", auth="public", website=True)
    def tests_view(self, test_id):
        ...
        test_result_ids = (
            request.env["lab.test.result"]
            .sudo()
            .search([("id", "in", all_test.result_ids.ids)])
        )
    ...
    @http.route("/my/op", type="http", auth="public", website=True)
    def portal_my_op(self, **kw):
        """Function for rendering prescriptions of portal user"""
        op = (
            request.env["hospital.outpatient"]
            .sudo()
            .search_read(
        ...
        values = {"op": op, "page_name": "op"}
        return request.render("base_hospital_management.portal_my_op", values)
