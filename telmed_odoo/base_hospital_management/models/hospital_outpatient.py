...
class HospitalOutpatient(models.Model):
    ...
    op_date = fields.Date(default=fields.Date.today(), string="Date", help="Date of OP")
    ...
    test_ids = fields.One2many(
        "lab.test.line", "op_id", string="Tests", help="Tests for the patient"
    )
    ...
    invoiced = fields.Boolean(string="Invoiced", help="True for invoiced")
    invoice_id = fields.Many2one(
        "account.move", copy=False, string="Invoice", help="Invoice of the patient"
    )
    attachment_id = fields.Many2one(
        "ir.attachment",
        string="Attachment",
        help="Attachments related to the outpatient",
    )
    active = fields.Boolean(
        string="Active", help="True for active patients", default=True
    )
    slot = fields.Float(
        string="Slot", help="Slot for the patient", copy=False, readonly=True
    )
    is_sale_created = fields.Boolean(
        string="Sale Created", help="True if sale order created"
    )
    ...
    def action_row_click_data(self, op_reference):
        ...
        op_record = (
            ...
            .sudo()
            .search(
                [("op_reference", "=", op_reference), ("active", "in", [True, False])]
            )
        )
        op_data = [
            op_reference,
            op_record.patient_id.patient_seq,
            op_record.patient_id.name,
            str(op_record.op_date),
            op_record.slot,
            op_record.reason,
        ...
        return {"op_data": op_data, "medicines": medicines}
    ...
    def create_new_out_patient(self, kw):
        ...
        if kw["id"]:
            partner = (
                ...
                .sudo()
                .search(
                    ["|", ("barcode", "=", kw["id"]), ("phone", "=", kw["op_phone"])]
                )
            )
            self.sudo().create(
                {
    ...
    def action_view_test(self):
        ...
        return {
            ...
            "target": "current",
            "type": "ir.actions.act_window",
            "domain": [("patient_type", "=", "outpatient"), ("op_id", "=", self.id)],
        }
