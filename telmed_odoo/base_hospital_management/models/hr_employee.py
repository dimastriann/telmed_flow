...
class HrEmployee(models.Model):
    ...
    currency_id = fields.Many2one(
        "res.currency",
        string="Currency",
        help="Currency in which consultation fee is calculating",
        default=lambda self: self.env.user.company_id.currency_id.id,
        required=True,
    )
    coach_id = fields.Many2one("hr.employee", string="Coach", help="Name of the coach")
    consultancy_charge = fields.Monetary(
        string="Consultation Charge", help="Charge for consultation"
    )
    consultancy_type = fields.Selection(
        [("resident", "Residential"), ("special", "Specialist")],
        string="Consultation Type",
        help="Select the type of Consultation",
    )
    time_avg = fields.Float(
        string="Average Time for a Patient",
        help="Average Consultation time per Patient in minutes",
    )
    degree_ids = fields.Many2many(
        "hospital.degree", string="Degree", help="Degrees of staff"
    )
    pharmacy_id = fields.Many2one(
        "hospital.pharmacy", string="Pharmacy", help="Name of the pharmacy"
    )
    specialization_ids = fields.Many2many(
        "doctor.specialization",
        string="Specialization",
        help="Doctors specialization for an area",
    )
