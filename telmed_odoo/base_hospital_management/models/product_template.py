...
class ProductTemplate(models.Model):
    ...
    medicine_brand_id = fields.Many2one(
        "medicine.brand",
        string="Brand",
        help="Indicates the brand of medicine or vaccine",
    )
    ...
    def action_get_medicine_data(self):
        ...
        medicines = []
        for rec in (
            self.env["product.template"].sudo().search([("medicine_ok", "=", True)])
        ):
            medicines.append(
    ...
    def action_get_vaccine_data(self):
        ...
        vaccines = []
        for rec in (
            self.env["product.template"].sudo().search([("vaccine_ok", "=", True)])
        ):
            vaccines.append(
