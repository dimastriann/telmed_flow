...
class HospitalWard(models.Model):
    ...
    building_id = fields.Many2one(
        "hospital.building",
        string="Block",
        help="The building to which the ward corresponds to",
    )
    floor_no = fields.Integer(
        string="Floor No.", help="The floor to which the ward corresponds to"
    )
    note = fields.Text(string="Note", help="Note regarding the ward")
    bed_count = fields.Integer(
        string="Count", compute="_compute_bed_count", help="The bed count"
    )
    ...
    ward_facilities_ids = fields.Many2many(
        "room.facility", string="Facilities", help="Facilities corresponds to ward."
    )
    ...
    def _compute_bed_count(self):
        ...
        for rec in self:
            rec.bed_count = (
                rec.env["hospital.bed"]
                .sudo()
                .search_count([("ward_id", "=", rec.ward_no)])
            )
