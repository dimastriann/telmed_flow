...
class LabTestLine(models.Model):
    ...
    date = fields.Date(default=fields.Date.today(), string="Date", help="Date of test")
    test_ids = fields.Many2many("lab.test", string="Test", help="All the tests")
    patient_type = fields.Selection(
        selection=[("inpatient", "Inpatient"), ("outpatient", "Outpatient")],
        required=True,
        string="Patient Type",
        help="Choose the patient type",
    )
    op_id = fields.Many2one(
        "hospital.outpatient", string="OP Number", help="ID of outpatient"
    )
    ip_id = fields.Many2one(
        "hospital.inpatient", string="Inpatient ID", help="ID of inpatient"
    )
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get("name", "New") == "New":
                vals["name"] = (
                    self.env["ir.sequence"].next_by_code("lab_tests.draft.sequence")
                    or "New"
                )
