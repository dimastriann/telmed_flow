...
class ResPartner(models.Model):
    ...
    date_of_birth = fields.Date(
        string="Date of Birth", help="Date of birth of the patient"
    )
    ...
    notes = fields.Html(
        string="Note", help="Notes regarding the notes", sanitize_style=True
    )
    ...
    barcode_png = fields.Binary(
        string="Barcode PNG", help="Image file of the barcode", readonly=True
    )
    group = fields.Selection(
        ...
        string="Ethnic Group",
        help="Specify your religion",
    )
    risk = fields.Text(string="Genetic Risks", help="Genetic risks of the patient")
    insurance_id = fields.Many2one(
        "hospital.insurance", string="Insurance", help="Patient insurance"
    )
    unique_id = fields.Char(
        string="Unique ID", help="Unique identifier to fetch patient insurance data"
    )
    family_ids = fields.One2many(
        "hospital.family", "family_id", string="Family ID", help="Family of a patient"
    )
    ...
    economic_level = fields.Selection(
        selection=[
            ("low", "Lower Class"),
            ("middle", "Middle Class"),
            ("upper", "Upper Class"),
    ...
    house_level = fields.Selection(
        selection=[("good", "Good"), ("bad", "Bad"), ("poor", "Poor")],
        string="House Condition",
        help="Specify your house's condition",
    )
    work_home = fields.Boolean(
        string="Work At Home", help="True if you are working from home"
    )
    hours_outside = fields.Integer(
        string="Hours Stay Outside Home",
        help="Specify how many hours you stay away from home",
    )
    hostile = fields.Boolean(
        string="Hostile Area", help="Specify your house in a friendly neighbourhood "
    )
    income = fields.Monetary(string="Income", help="The in come of patient")
    currency_id = fields.Many2one(
        "res.currency",
        string="Currency",
        help="Currency in which invoices and payments will be generated",
        default=lambda self: self.env.user.company_id.currency_id.id,
        required=True,
    )
    sanitary = fields.Boolean(
        "Sanitary Sewers",
    ...
    running = fields.Boolean(
        string="Running Water",
        help="water that comes into a building through "
        "pipes. A cabin with hot and cold running "
        "water.",
    )
    electricity = fields.Boolean(
        string="Electricity", help="True if you have electricity"
    )
    gas = fields.Boolean(string="Gas Supply", help="True if you have gas supply")
    trash = fields.Boolean(
        string="Trash Collection", help="True if you have trash collection"
    )
    home_phone = fields.Boolean(string="Telephone", help="True if you have telephone")
    tv = fields.Boolean(string="Television", help="True if you have television")
    internet = fields.Boolean(string="Internet", help="True if you have internet")
    help = fields.Selection(
        [("yes", "Yes"), ("no", "No")],
        string="Family Help",
        help="Specify whether your family is willing to help or not",
    )
    discussion = fields.Selection(
        [("yes", "Yes"), ("no", "No")],
        string="Family Discussion ",
        help="Specify your family have a good discussion at home ",
    )
    ability = fields.Selection(
        [("very", "Very good"), ("good", "Good"), ("bad", "Bad"), ("poor", "Poor")],
        string="Family Ability",
        help="family status of the patient",
    )
    time_sharing = fields.Selection(
        [("yes", "Yes"), ("no", "No")],
        string=" Family Time Sharing ",
        help="Specify your family share time at home ",
    )
    affection = fields.Selection(
        [("very", "Very good"), ("good", "Good"), ("bad", "Bad"), ("poor", "Poor")],
        string="Family Affection ",
        help="Specify your family's affection ",
    )
    single = fields.Boolean(
        string="Single Parent Family", help="Whether single parent family or not"
    )
    violence = fields.Boolean(
        string="Domestic Violence", help="True if you are facing any domestic violence"
    )
    children = fields.Boolean(
        string="Working Children", help="Do you have working children"
    )
    abuse = fields.Boolean(string="Sexual Abuse", help="Do you faced any sexual abuse")
    drug = fields.Boolean(string="Drug Addiction", help="Do you have drug addiction")
    withdrawal = fields.Boolean(
        string="Withdrawal", help="Do you faced any withdrawal symptoms"
    )
    in_prison = fields.Boolean(
        string="Has Been In Prison", help="True if you had been in prison"
    )
    current_prison = fields.Boolean(
        string="Currently In Prison", help="True if you are in prison currently"
    )
    relative_prison = fields.Boolean(
        string="Relative In Prison", help="True if any of your relative is in prison"
    )
    hospital_vaccination_ids = fields.One2many(
        "hospital.vaccination",
        "patient_id",
        string="Vaccination",
        help="Vaccination details of patient",
    )
    fertile = fields.Boolean(
        string="Fertile",
        help="""Capable of developing 
                                             into a complete organism; 
                                             fertilized. Capable of supporting 
                                             plant life; favorable to the 
                                             growth of crops and plants.""",
    )
    menarche_age = fields.Integer(
        string="Menarche Age",
        help="""The first 
    ...
    pause = fields.Boolean(
        string="Menopause",
        help="""Menopause is a point in 
                                 time 12 months after a woman's last period""",
    )
    pause_age = fields.Integer(
        string="Menopause Age", help="Age at which menopause occurred"
    )
    pap = fields.Boolean(
        string="PAP Test",
        help="""
    ...
    self = fields.Boolean(
        string="Self breast examination",
        help="A breast self-exam for breast awareness is "
        "in inspection "
        "of your breasts that women do on your own",
    )
    mommography = fields.Boolean(
        string="Mommography", help="Mammograms can be used to look for breast cancer"
    )
    last_pap = fields.Date(
        string="Last PAP Test", help="The date on which last PAP test has been done"
    )
    last_col = fields.Date(
        string="Last Colposcopy", help="The date on which last colposcopy has been done"
    )
    deceased = fields.Boolean(
        string="Deceased during 1st week",
        help="The family member deceased during first week",
    )
    grandiva = fields.Boolean(string="Grandiva", help="True for grandiva")
    alive = fields.Boolean(string="Born Alive", help="Whether born alive or not")
    ...
    abortions = fields.Integer(
        string="No Of Abortions", help="Number of abortions of patient"
    )
    exercise = fields.Boolean(
        string="Exercise", help="True if patient doing exercise regularly"
    )
    minute = fields.Integer(
        string="Minute/Day", help="The duration of exercise per day"
    )

    day_sleep = fields.Boolean(
        string="Sleeps At Daytime", help="True if sleeps at daytime"
    )
    ...
    coffee = fields.Boolean(
        string="Coffee", help="True if you have a habit of drinking coffee"
    )
    cup = fields.Integer(string="Cups/Day", help="Number of cups of coffee per day")
    drink = fields.Boolean(string="Soft Drink", help="True if you drinks soft drinks")
    salt = fields.Boolean(string="Salt", help="True if you use salt")
    diet = fields.Boolean(
        string="Currently On Diet", help="True if you are on diet currently"
    )
    ...
    age_start = fields.Integer(
        string="Age of Started Smoking", help="Age on which you started your smoking"
    )
    cigarettes = fields.Integer(
        string="Cigarettes/Day", help="Number of cigarettes per day"
    )
    passive = fields.Boolean(string="Passive Smoker", help="True for passive smokers")
    age_quit = fields.Integer(
        string="Age of Quitting", help="Age at which you quit your smoking habit"
    )
    alcoholic = fields.Boolean(string="Alcoholic", help="True for alcoholics")
    ex_alcoholic = fields.Boolean(string="Ex-Alcoholic", help="True for ex- alcoholics")
    age_start_alco = fields.Integer(
        string="Age to Start Drinking",
        help="Age at which you started your drinking habit",
    )
    beer = fields.Integer(string="Beer/Day", help="Number of beers per day")
    liquor = fields.Integer(string="Liquor/Day", help="Liquors per day")
    wine = fields.Integer(string="Wine/Day", help="Number of wines per day")
    age_quit_alcoholic = fields.Integer(
        string="Age Of Quitting", help="Age at which you started your drinking habit"
    )
    drugs = fields.Boolean(string="Drug User", help="True for drug users")
    ex_drugs = fields.Boolean(string="Ex-Drug User", help="True for Ex drug user")
    iv_user = fields.Boolean(string="IV Drug User", help="True for IV drug user")
    age_start_drug = fields.Integer(
        string="Age to Start Using Drugs", help="Age at which you started using drug"
    )
    age_quit_drug = fields.Integer(
        string="Drug Quitting Age", help="Age of quitting drug"
    )
    orientation = fields.Selection(
        [("straight", "Straight"), ("homo", "Homosexual"), ("trans", "Trans-Gender")],
        string="Orientation",
    )
    age_sex = fields.Integer(
        string="Age of First Encounter", help="Age of first sex encounter"
    )
    partners = fields.Integer(string="No of Partners", help="Number of sex partners")
    anti = fields.Selection(
        ...
        help="Choose your contraceptive method",
    )
    oral = fields.Boolean(string="Oral Sex", help=("uttered by the mouth or in words"))
    anal = fields.Boolean(
        string="Anal Sex", help="True if you are encountering anal sex"
    )
    prostitute = fields.Boolean(string="Prostitute", help="True for prostitutes")
    prostitute_sex = fields.Boolean(
        string="Sex With Prostitute",
        help="True if you are encountered sex with prostitute",
    )
    sex_notes = fields.Text(string="Notes", help="Write down the notes")
    rider = fields.Boolean(string="Motorcycle Rider", help="True for motorcycle riders")
    helmet = fields.Boolean(
        string="Uses Helmet", help="True if you regularly use helmet"
    )
    laws = fields.Boolean(
        string="Obey Traffic Laws", help="True if you obey traffic rules"
    )
    revision = fields.Boolean(
        string="Car Revision", help="True if car revision is done"
    )
    belt = fields.Boolean(
        string="Seat Belt", help="True if you uses seat belt regularly"
    )
    safety = fields.Boolean(
        string="Car Child Safety", help="True if you have car child safety"
    )
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get("patient_seq", "New") == "New":
                vals["patient_seq"] = (
                    self.env["ir.sequence"].next_by_code("patient.sequence") or "New"
                )
    ...
    def action_generate_patient_card(self):
        ...
        company = (
            ...
            .sudo()
            .search([("id", "=", self.env.context["allowed_company_ids"])])
        )
        data = {
            "name": self.name,
    ...
    def reception_op_phone(self, phone):
        ...
        patient_phone = self.sudo().search([("phone", "=", phone["patient-phone"])])
        return {
    ...
    def action_get_patient_data(self, patient_id):
        ...
        data = self.sudo().search(
            ["|", ("patient_seq", "=", patient_id), ("barcode", "=", patient_id)]
        )
        patient_history = []
        for rec in (
            self.env["hospital.outpatient"]
            .sudo()
            .search([("patient_id", "=", data.id)])
        ):
            patient_history.append(
                [rec.op_reference, str(rec.op_date), rec.doctor_id.doctor_id.name]
            )
        values = {
            "name": data.name,
    ...
    def create_sale_order_pharmacy(self, order):
        ...
        op_record = (
            ...
            .sudo()
            .search(
                [("op_reference", "=", order), "|", ("active", "in", [False, True])]
            )
        )
                ...
                self.env["hospital.outpatient"].sudo().create(
                    {
                        ...
                        "reason": post["reason"],
                        "slot": post["slot"],
                        "doctor_id": self.env["doctor.allocation"]
                        .sudo()
                        .browse(post["doctor"])
                        .id,
                    }
