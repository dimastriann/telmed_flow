...
class PatientRoom(models.Model):
    ...
    currency_id = fields.Many2one(
        ...
        required=True,
        help="Currency in which payments will be done",
    )
    building_id = fields.Many2one(
        "hospital.building",
        string="Block Name",
        required=True,
        help="Building to which the room corresponds to",
    )
    ...
    room_facilities_ids = fields.Many2many(
        "room.facility", string="Facilities", help="Facilities of room"
    )
    floor_no = fields.Integer(
        string="Floor No.", help="The floor to which the room corresponds to"
    )
    ...
    @api.model
    def fetch_rooms_details(self):
        """Function for fetch room details"""
