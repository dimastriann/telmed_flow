...
class PatientLabTest(models.Model):
    ...
    test_id = fields.Many2one("lab.test.line", string="Test", help="Name of the test")
    ...
    patient_type = fields.Selection(
        selection=[("inpatient", "Inpatient"), ("outpatient", "Outpatient")],
        related="test_id.patient_type",
        string="Type",
        help="Choose the type of patient",
    )
    test_ids = fields.Many2many(
        "lab.test",
        related="test_id.test_ids",
        string="Tests",
        help="All the tests added for the patient",
    )
    date = fields.Date(string="Date", help="Date of test", default=fields.Date.today())
    total_price = fields.Float(string="Price", help="Total price for the test")
    currency_id = fields.Many2one(
        "res.currency",
        string="Currency",
        help="Currency in which lab test amount will be calculated",
        default=lambda self: self.env.user.company_id.currency_id.id,
        required=True,
    )
    state = fields.Selection(
        [("draft", "Draft"), ("test", "Test In Progress"), ("completed", "Completed")],
        string="State",
        readonly=True,
        help="State of test",
        default="draft",
    ...
    result_ids = fields.One2many(
        "lab.test.result", "parent_id", string="Result", help="Results of the test"
    )
    ...
    lab_id = fields.Many2one(
        "hospital.laboratory", string="Lab", help="Lab in which test is doing"
    )
    invoice_id = fields.Many2one(
        "account.move", string="Invoice", help="Invoice for the test", copy=False
    )
    order = fields.Integer(
        string="Sale Order",
        help="Number of sale orders for vaccines and medicines",
        copy=False,
    )
    started = fields.Boolean(
        string="Test Started", help="True if the test has been started", copy=False
    )
    invoiced = fields.Boolean(
        string="Invoiced", help="True if the test has been invoiced", copy=False
    )
    sold = fields.Boolean(
        string="Sold",
        help="If true, sale order smart button will be visible ",
        copy=False,
    )
    ...
    invoice_count = fields.Integer(
        ...
        help="Total number of invoices for this patient lab test.",
    )
    sale_count = fields.Integer(
        string="Sale Count",
        compute="_compute_sale_count",
        help="Total number of sale orders for this patient lab test.",
    )
    inpatient_id = fields.Many2one(
        "hospital.inpatient", string="Inpatient", help="Choose the inpatient"
    )
            ...
            record.invoice_count = (
                ...
                .search_count(
                    [
                        ...
                        ("payment_reference", "=", record.test_id.name),
                    ]
                )
            )
    ...
    def _compute_sale_count(self):
        ...
        for record in self:
            record.sale_count = (
                self.env["sale.order"]
                .sudo()
                .search_count([("reference", "=", record.test_id.name)])
            )
        ...
        if data:
            ...
            if data.patient_id.blood_group:
                ...
                patient_data["blood_group"] = (
                    blood_caps + str(data.patient_id.rh_type),
                )
    ...
    def create_invoice(self, rec_id):
                ...
                order_lines.append(
                    (
                        ...
                        0,
                        {
                            ...
                            .sudo()
                            .search([("product_tmpl_id", "=", rec.medicine_id.id)])
                            .id,
        ...
        invoice_id = (
            self.env["account.move"]
            .sudo()
            .search([("ref", "=", data.test_id.name)], limit=1)
        )
        if not invoice_id:
            invoice_id = (
                self.env["account.move"]
    ...
    def action_view_sale_order(self):
        ...
        sale_order = (
            self.env["sale.order"]
            .sudo()
            .search([("partner_id", "=", self.patient_id.id)], limit=1)
        )
        return {
            "name": "Sale order",
    ...
    def print_lab_tests(self):
        ...
        return self.env.ref(
            "base_hospital_management.action_report_patient_lab_tests"
