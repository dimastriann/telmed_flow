...
class HospitalPharmacy(models.Model):
    ...
    name = fields.Char(string="Name", help="Name of the pharmacy", required=True)
    ...
    state_id = fields.Many2one(
        "res.country.state", string="State", help="State of pharmacy"
    )
    country_id = fields.Many2one(
        "res.country", string="Country", help="Country of pharmacy"
    )
    notes = fields.Text(string="Notes", help="Notes regarding pharmacy")
    image_129 = fields.Image(
        string="Image", help="Image of pharmacy", max_width=128, max_height=128
    )
    active = fields.Boolean(
        string="Active", help="True for active pharmacy", default=True
    )
    medicine_ids = fields.One2many(
        "pharmacy.medicine",
        "pharmacy_id",
        string="Pharmacy",
        help="Indicates the medicines in the pharmacy",
    )
    sales_team_id = fields.Many2one(
        "crm.team", string="Sales Team", help="Choose the sales-team for the pharmacy"
    )
    ...
    def create_sale_order(self, kwargs):
                    ...
                    "order_line": [
                        (
                            ...
                            0,
                            {
                                ...
                                .search([("product_tmpl_id", "=", int(rec["product"]))])
                                .id,
                                "product_uom_qty": float(rec["qty"]),
                                "price_unit": float(rec["price"])
                                if "price" in rec.keys()
        ...
        if "op" in kwargs.keys():
            self.env["hospital.outpatient"].sudo().search(
                [("op_reference", "=", kwargs["op"])]
            ).write({"is_sale_created": True})
        return {
            "invoice_id": pharmacy_sale_order.id,
            "invoice": pharmacy_sale_order.name,
        }
    ...
    def tax_amount(self, kw):
        ...
        return {"amount": self.env["account.tax"].sudo().browse(kw).amount}
    ...
    def fetch_sale_orders(self):
        ...
        return self.env["sale.order"].search_read(
            ...
            fields=["name", "create_date", "partner_id", "amount_total", "state"],
        )
