...
class InpatientSurgery(models.Model):
    ...
    name = fields.Char(string="Name", help="Name of the surgery", required=True)
    date = fields.Date(
        dafault=fields.Date.today(), string="Date", help="Date of adding surgery"
    )
    planned_date = fields.Datetime(
        string="Planned Date", help="Planned date for surgery", required=True
    )
    ...
    hours_to_take = fields.Float(
        string="Duration", help="Time duration for the surgery"
    )
    ...
    def get_doctor_slot(self):
        ...
        for rec in self.sudo().search([("doctor_id.user_id", "=", self.env.user.id)]):
            data_list.append(
                {
                    "id": rec.id,
