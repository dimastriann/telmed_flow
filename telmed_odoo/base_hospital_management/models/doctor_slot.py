...
class DoctorSlot(models.Model):
    ...
    date_start = fields.Datetime(
        default=fields.Date.today(), string="Date", help="Date of surgery"
    )
    ...
    def hospital_inpatient_cancel(self):
        ...
        for rec in (
            ...
            .sudo()
            .search(
                [("doctor_id.user_id", "=", self.env.user.id), ("state", "=", "cancel")]
            )
        ):
            data_list.append(
                {
    ...
    def hospital_inpatient_done(self):
        ...
        for rec in (
            ...
            .sudo()
            .search(
                [("doctor_id.user_id", "=", self.env.user.id), ("state", "=", "done")]
            )
        ):
            data_list.append(
                {
    ...
    def action_get_doctor_slot(self):
        ...
        for rec in self.sudo().search(
            [("doctor_id.user_id", "=", self.env.user.id), ("state", "=", "draft")]
        ):
            data_list.append(
                {
                    "id": rec.id,
