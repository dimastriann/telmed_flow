...
class HospitalVaccination(models.Model):
    ...
    name = fields.Char(
        string="Vaccination Reference",
        copy=False,
        readonly=True,
        index=True,
        help="Name of vaccination",
    ...
    vaccine_product_id = fields.Many2one(
        ...
        string="Vaccine",
        help="Choose the vaccine",
        required=True,
    )
    vaccine_price = fields.Float(
        related="vaccine_product_id.list_price", string="Price", help="Price of vaccine"
    )
    sale_order_id = fields.Many2one(
        "sale.order", string="Sale Order", help="Sale order for the vaccine"
    )
    sold = fields.Boolean(string="Sold", help="True if sale order created")
    certificate = fields.Binary(string="Certificate", help="Vaccination certificate")
    attachment_id = fields.Many2one(
        "ir.attachment",
        string="Attachment",
        help="Attachments added to the vaccination",
    )
    recurring_vaccine = fields.Boolean(
        string="Recurring Vaccine", help="True for recurring vaccinations"
    )
    total_vaccine = fields.Integer(string="Total Dose", help="Total number of vaccines")
    next_vaccine_days = fields.Integer(
        string="Next Vaccine (In Days)", help="The number of days to next vaccine"
    )
    next_vaccine = fields.Date(
        string="Next Vaccination Date", help="Date of next vaccine", readonly=True
    )
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get("name", "New") == "New":
                vals["name"] = (
                    self.env["ir.sequence"].next_by_code("vaccination.sequence")
                    or "New"
                )
    ...
    def _onchange_next_vaccine_days(self):
        ...
        if self.next_vaccine_days:
            self.sudo().write(
                {
                    "next_vaccine": fields.Date.today()
                    + timedelta(self.next_vaccine_days)
                }
            )
    ...
    def action_create_so(self):
        ...
        product_id = (
            ...
            .sudo()
            .search([("product_tmpl_id", "=", self.vaccine_product_id.id)])
        )
        sale = self.env["sale.order"].search(
            [("partner_id.id", "=", self.patient_id.id), ("state", "=", "draft")],
            limit=1,
        )
        if sale:
            sale.sudo().write(
