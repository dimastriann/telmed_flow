...
class PrescriptionLine(models.Model):
    ...
    prescription_id = fields.Many2one(
        "hospital.prescription", string="Prescription", help="Name of the prescription"
    )
    ...
    quantity = fields.Integer(
        string="Quantity",
        required=True,
        help="The number of medicines for the time period",
    )
    no_intakes = fields.Float(
        string="Intakes", required=True, help="How much medicine want to take"
    )
    time = fields.Selection(
        [
            ("once", "Once in a day"),
            ("twice", "Twice in a Day"),
            ("thrice", "Thrice in a day"),
            ("morning", "In Morning"),
            ("noon", "In Noon"),
            ("evening", "In Evening"),
        ],
