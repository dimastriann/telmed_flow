...
class HospitalBuilding(models.Model):
    ...
    phone = fields.Char(string="Phone", help="Phone number for contact the building")
    mobile = fields.Char(string="Mobile", help="Mobile number for contact the building")
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get("name", "New") == "New":
                vals["name"] = (
                    self.env["ir.sequence"].next_by_code("building.sequence") or "New"
                )
    ...
    def _compute_room_count(self):
        ...
        for rec in self:
            rec.room_count = (
                self.env["patient.room"]
                .sudo()
                .search_count([("building_id", "=", rec.id)])
            )
