...
class HospitalInpatient(models.Model):
    ...
    name = fields.Char(
        ...
        readonly=True,
        index=True,
        help="Sequence number of inpatient for uniquely identifying",
    ...
    bed_id = fields.Many2one("hospital.bed", string="Bed", help="Choose the bed")
    room_id = fields.Many2one(
        "patient.room",
        string="Room",
        help="Choose the room to which patient admitted to",
    )
    ...
    type_admission = fields.Selection(
        [("emergency", "Emergency Admission"), ("routine", "Routine Admission")],
    ...
    operating_doctor_id = fields.Many2one(
        "hr.employee",
        string="Operating Doctor",
        help="Name of operating doctor",
        domain=[("job_id.name", "=", "Doctor")],
    )
    hosp_date = fields.Date(
        string="Admission Date",
        help="Date of hospitalisation",
        default=fields.Date.today(),
    )
    discharge_date = fields.Date(
        string="Discharge Date", help="Date of discharge", copy=False
    )
    condition = fields.Text(
        string="Condition Before Hospitalization",
        help="The condition of the patient before he/she is admitted to the hospital",
    )
    nursing_plan_ids = fields.One2many(
        "nursing.plan",
        "admission_id",
        string="Nursing Plan",
    ...
    active = fields.Boolean(
        string="Active", default=True, help="True for active inpatients"
    )
    doctor_round_ids = fields.One2many(
        "doctor.round",
        "admission_id",
    ...
    discharge_plan = fields.Text(
        string="Discharge Plan", help="Discharge plan of the inpatient"
    )
    notes = fields.Text(string="Notes", help="Notes regarding the inpatient")
    bed_rent = fields.Monetary(
        string="Rent Per Day", related="bed_id.bed_rent", help="Rent for the bed"
    )
    ...
    bed_rent_amount = fields.Monetary(
        string="Total Bed Rent",
        help="Total rent for stayed days",
        compute="_compute_bed_rent_amount",
        copy=False,
    )
    invoice_id = fields.Many2one(
        "account.move", string="Invoice", help="Invoice id of the inpatient", copy=False
    )
    admit_days = fields.Integer(
        string="Days",
    ...
    is_invoice = fields.Boolean(
        string="Is Invoice",
        help="View invoice button will be visible if this field is true",
    )
    ...
    enable_outpatient = fields.Boolean(
        string="Prescription History",
        help="If checked, the prescription history of the patient will be added",
    )
    ...
    room_rent = fields.Monetary(
        string="Rent per day", help="Rent for the room", related="room_id.rent"
    )
    room_rent_amount = fields.Monetary(
        string="Total Room Rent",
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get("name", "New") == "New":
                vals["name"] = (
                    self.env["ir.sequence"].next_by_code("hospital.inpatient") or "New"
                )
    ...
    def _compute_test_count(self):
        ...
        self.test_count = (
            self.env["lab.test.line"]
            .sudo()
            .search_count([("id", "in", self.test_ids.ids)])
        )
    ...
    def _compute_admit_days(self):
        ...
        if self.hosp_date:
            if self.discharge_date:
                self.admit_days = (
                    self.discharge_date - self.hosp_date + datetime.timedelta(days=1)
                ).days
            else:
                self.admit_days = (
                    fields.Date.today() - self.hosp_date + datetime.timedelta(days=1)
                ).days

    @api.depends("hosp_date", "discharge_date", "admit_days", "room_id")
    ...
    @api.depends("hosp_date", "discharge_date", "room_rent_amount", "admit_days")
    ...
    def _onchange_enable_outpatient(self):
        ...
        if self.enable_outpatient:
            outpatient_id = (
                ...
                .sudo()
                .search([("patient_id", "=", self.patient_id.id)])
            )
            self.sudo().write(
                {
                    "prescription_ids": [
    ...
    def _onchange_bed_type(self):
        ...
        return {
            "domain": {
                "bed_id": [("bed_type", "=", self.bed_type), ("state", "=", "avail")],
                "room_id": [("bed_type", "=", self.bed_type), ("state", "=", "avail")],
            }
        }
    ...
    def action_view_test(self):
        ...
        return {
            ...
            "target": "current",
            "type": "ir.actions.act_window",
            "domain": [("patient_type", "=", "inpatient"), ("ip_id", "=", self.id)],
        }
    ...
    def action_invoice(self):
        ...
        for rec in self:
            ...
            if invoice:
                for line in invoice.read(["name", "price_unit", "quantity"]):
                    inv_line_list.append(
                        (
                            0,
                            0,
    ...
    def create_new_in_patient(self, domain):
        ...
        if domain:
            patient = (
                self.env["hospital.inpatient"]
                .sudo()
                .search([("patient_id", "=", domain["patient_id"])])
            )
            if patient:
