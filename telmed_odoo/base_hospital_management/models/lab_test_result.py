...
class LabTestResult(models.Model):
    ...
    uom_id = fields.Many2one(
        "uom.uom", string="Unit", help="Unit of the normal and result value"
    )
    parent_id = fields.Many2one(
        "patient.lab.test",
        string="Tests",
        help="The tests for which the result corresponds to",
    )
    test_id = fields.Many2one("lab.test", string="Test Name", help="Name of the test")
    ...
    price = fields.Monetary(
        string="Cost", help="Cost for the test", related="test_id.price"
    )
    tax_ids = fields.Many2many("account.tax", string="Tax", help="Tax for the test")
    state = fields.Selection(
        selection=[("processing", "Processing"), ("published", "Published")],
        string="State",
        help="State of the result",
        default="processing",
