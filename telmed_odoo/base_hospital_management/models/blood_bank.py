...
class BloodBank(models.Model):
    ...
    date = fields.Date(
        string="Date", help="Blood donating date", default=fields.Date.today()
    )
    ...
    blood_donation_ids = fields.One2many(
        "blood.donation",
        "blood_bank_id",
    ...
    state = fields.Selection(
        [("avail", "Available"), ("not", "Unavailable")],
        string="State",
        help="State of the blood donation",
        readonly=True,
        default="avail",
    )
    assigned_patient_id = fields.Many2one(
        ...
        string="Receiver",
        domain=[("patient_seq", "not in", ["New", "Employee", "User"])],
        help="Choose the patient to whom blood is donating",
    )
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get("name", "New") == "New":
                vals["name"] = (
                    self.env["ir.sequence"].next_by_code("blood.bank") or "New"
                )
    ...
    def _onchange_partner_id(self):
        ...
        self.sudo().write(
            {
                "blood_donation_ids": [
                    (0, 0, {"questions": rec.blood_donation_question})
                    for rec in self.env["contra.indication"].sudo().search([])
                ]
            }
        )
