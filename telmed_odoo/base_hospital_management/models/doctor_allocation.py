...
class DoctorAllocation(models.Model):
    ...
    name = fields.Char(
        string="Name", readonly=True, default="New", help="Name of the allocation"
    )
    ...
    op_ids = fields.One2many(
        ...
        "doctor_id",
        string="Booking",
        help="Patient booking belongs to this allocation",
    )
    ...
    work_from = fields.Float(
        string="Work From",
        required=True,
        help="Allocation time starting time in 24 hrs format",
    )
    time_avg = fields.Float(
        string="Duration",
        help="Average Consultation time per Patient in minutes",
        readonly=False,
    ...
    patient_limit = fields.Integer(
        string="Limit Patient",
        help="Maximum number of patients allowed per allocation",
        store=True,
        compute="_compute_patient_limit",
    )
    patient_count = fields.Integer(
        string="Patient Count",
        help="Number of patient under this allocation",
        compute="_compute_patient_count",
    )
    slot_remaining = fields.Integer(
        string="Slots Remaining",
        help="Number of slots remaining in this allocation",
        store=True,
        compute="_compute_slot_remaining",
    )
    latest_slot = fields.Float(
        string="Available Slot", help="Indicates the latest available slot"
    )
    state = fields.Selection(
        [("draft", "Draft"), ("confirm", "Confirmed"), ("cancel", "Cancelled")],
        default="draft",
        string="State",
        help="State of Doctor allocation",
    )
        ...
        for vals in vals_list:
            ...
            if doctor_group in self.env.user.group_ids:
                ...
                if default_doctor_id:
                    vals["name"] = (
                        default_doctor_id.name + ": " + work_from + "-" + work_to
                    )
        ...
        for allocation in self:
            ...
            if overlapping_allocations:
                raise ValidationError(
                    "Overlap detected with another doctor allocation on the same date."
                )
    ...
    def _compute_patient_limit(self):
        ...
        for record in self:
            if (
                record.work_from is not None
                and record.work_from != 0
                and record.work_to is not None
                and record.work_to != 0
                and record.time_avg
            ):
